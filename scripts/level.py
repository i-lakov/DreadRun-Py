...
class Level:
    ...
    def create_explosion(self, pos):
        ...
        self.explosion_sprite.add(explosion_sprite_animation)

        for enemy_indiv in self.enemies:
            enemy = enemy_indiv.sprite
            if self.explosion_sprite.sprite.rect.colliderect(enemy.rect):
    ...
    def setup(self):
        ...
        for r_index, row in enumerate(self.level):
            for b_index, box in enumerate(row):
                ...
                if box == 'G':
                    ...
                    player_sprite = Player((y, x), self.display_surface, self.create_explosion)
        ...
        for sprite in self.tiles.sprites():
            ...
            if self.should_collide(sprite.get_name()) and sprite.rect.colliderect(player.rect):
                if player.direction.x < 0:  # leftwards collision
                    ...
                    player.on_left = True
                    self.cur_x = player.rect.left
                if player.direction.x > 0:  # rightwards collision
                    player.rect.right = sprite.rect.left
                    player.on_right = True
                    self.cur_x = player.rect.right

        if player.on_left and (player.rect.left < self.cur_x or player.direction.x >= 0):
            player.on_left = False
        if player.on_right and (player.rect.right > self.cur_x or player.direction.x <= 0):
    ...
    def draw(self):
        ...
        self.player.update()
        for enemy_indiv in self.enemies:
            enemy_indiv.update(self.world_shift)
        self.horizontal_collision()
        self.vertical_collision()
        self.player.draw(self.display_surface)
        for enemy_indiv in self.enemies:
            enemy_indiv.draw(self.display_surface)
    ...
    def level_generator(self):
        ...
        for x in range(self.chunk_size):
            ...
            self.level.append(column)
        
        self.level[player_start_pos][player_block_height] = 'P'     # player
        
        # boxes to prevent you from accidentally falling at start
        for i in range(player_start_pos):
            for j in range(8):
                self.level[i][first_block_height + 3 - j] = 'B' 
